 ...
 *
 * <p>Exposes aggregate counts and helper accessors over the underlying
 * {@link DataDiff} so callers can assert on match percentages, record-level
 * modifications, and generate reports without depending on internal classes.
 */
@Getter
public class DiffResult {
    ...
    /**
     * Percentage of left-side rows that matched a right-side row without
     * modification. Uses the larger of left/right as denominator to avoid
     * inflating the score when one side is shorter.
     */
    ...
    /**
     * Lightweight view over a modified {@link DiffRow} that surfaces
     * field-level changes to test code.
     */
    @Getter
