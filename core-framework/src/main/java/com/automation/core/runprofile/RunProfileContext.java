...
/**
 * Thread-safe singleton that caches the active {@link RunProfile} for the
 * current JVM.
 *
 * <p>The profile name is resolved once, in order of precedence:
 * <ol>
 *   <li>System property {@code -Dprofile=...}</li>
 *   <li>System property {@code -Drun.profile=...}</li>
 *   <li>Environment variable {@code RUN_PROFILE}</li>
 *   <li>Default: {@code local}</li>
 * </ol>
 *
 * <p>Unlike {@link com.automation.core.environment.EnvironmentContext}, the
 * run profile is intentionally optional: if no {@code profiles/local.json} is
 * present on the classpath the context falls back to an in-memory default
 * profile so tests that don't care about profiles still work.
 */
@Log4j2
public final class RunProfileContext {
        ...
        if (local == null) {
            synchronized (RunProfileContext.class) {
                ...
                if (local == null) {
                    ...
                    instance = local;
                    log.info("RunProfileContext initialised for '{}' "
    ...
    private static RunProfile tryLoad(String name) {
        try {
            ...
            log.warn("No run profile '{}' found - falling back to built-in defaults. ({})",
                name, notFound.getMessage());
    ...
    /**
     * Absolute path to an external directory holding environment JSON files,
     * or {@code null} when the active profile doesn't override the default
     * classpath location.
     */
    ...
    private Path outputDir(java.util.function.Function<OutputDirs, String> extractor, String fallback) {
