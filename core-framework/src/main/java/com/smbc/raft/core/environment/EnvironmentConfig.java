...
/**
 * Root POJO for a single environment's JSON configuration (e.g. dev.json,
 * qa.json, uat2.json). Groups named databases, message queues and websites so
 * that a test targeting {@code dev} has isolated access to exactly the
 * connections provisioned for dev.
 *
 * <p>Deliberately agnostic of any specific application. Application-specific
 * test projects author the JSON files; the core only knows how to bind them.
 */
@Data
@NoArgsConstructor
