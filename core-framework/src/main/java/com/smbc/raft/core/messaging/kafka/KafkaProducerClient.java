...
/**
 * Thin wrapper around {@link KafkaProducer} that accepts {@link Message} values and returns the
 * broker-assigned metadata back onto the message.
 */
@Log4j2
public class KafkaProducerClient implements AutoCloseable {
  ...
  /**
   * Send a message synchronously and populate the broker-assigned fields (partition, offset,
   * timestamp) on the original {@link Message}.
   */
  ...
  public Message send(Message message, long timeout, TimeUnit unit) {
    ...
    try {
      ...
      throw new MessagingException(
          "Failed to send Kafka message to " + message.getDestination(), e);
    }
  }
