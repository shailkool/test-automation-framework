 ...
 *
 * <p>Re-invokes an action until it satisfies a predicate, up to a configured
 * number of iterations. Returns a {@link ValidationResult} summarising the
 * outcome. Designed for UI, API, and database scenarios where the expected
 * state takes time to propagate.
 */
@Log4j2
@Getter
public class ValidationLoop {
    ...
    /**
     * Poll {@code actualSupplier} until the {@code matcher} returns true when
     * applied to the supplied value and {@code expected}.
     */
    public <A, E> ValidationResult<A> validate(Supplier<A> actualSupplier,
                                               BiPredicate<A, E> matcher,
                                               E expected) {
        ...
        for (int i = 0; i < config.getIterations(); i++) {
            ...
            try {
                ...
                log.warn("Iteration {}/{} threw: {}",
                    iteration, config.getIterations(), e.getMessage());
            }
        ...
        ValidationResult<A> result = new ValidationResult<>(
            null,
            actual,
            success,
            iteration,
    ...
    public ValidationResult<String> validateContains(Supplier<String> supplier, String expected) {
        return validate(
            supplier,
            (actual, exp) -> actual != null && exp != null && actual.contains(exp),
            expected
        );
    }
