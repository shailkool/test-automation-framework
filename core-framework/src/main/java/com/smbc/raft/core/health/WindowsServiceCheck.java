 ...
 *
 * <p>Uses {@code sc [\\host] query <serviceName>} — no WMI, no extra
 * dependencies, works from any JDK 17 process that has network access
 * to the target host.
 *
 * <p>On non-Windows agents (Linux CI containers) the check returns
 * {@link HealthCheckResult.Status#SKIPPED} so the same suite
 * runs unchanged in Docker without platform guards in test code.
 *
 * <p>Usage:
 * <pre>
    ...
    public HealthCheckResult run() {
        ...
        if (!isWindows() && host.equalsIgnoreCase("localhost")) {
            return HealthCheckResult.builder()
                .name(checkName).category("WindowsService")
                .status(HealthCheckResult.Status.SKIPPED)
                .detail("Non-Windows agent — service check skipped")
                .durationMillis(0).checkedAt(Instant.now()).build();
        }

        try {
            String[] cmd = host.equalsIgnoreCase("localhost")
            ...
            if (out.contains("FAILED 1060") || out.contains("does not exist")) {
                return result(HealthCheckResult.Status.DOWN,
                    "Service '" + serviceName + "' not found on " + host, elapsed);
            }
            if (out.contains("RUNNING")) {
                return result(HealthCheckResult.Status.UP,
                    serviceName + " is RUNNING on " + host, elapsed);
            }
            if (out.contains("STOPPED")) {
                return result(HealthCheckResult.Status.DOWN,
                    serviceName + " is STOPPED on " + host, elapsed);
            }
            if (out.contains("STOP_PENDING") || out.contains("START_PENDING")) {
                return result(HealthCheckResult.Status.DEGRADED,
                    serviceName + " is transitioning on " + host, elapsed);
            }
            return result(HealthCheckResult.Status.DEGRADED,
                "Unexpected sc output: " + out.trim(), elapsed);

        } catch (Exception e) {
            long elapsed = System.currentTimeMillis() - start;
            log.error("Service check failed for {} on {}: {}", serviceName, host, e.getMessage());
            return result(HealthCheckResult.Status.DOWN,
                "Error querying service: " + e.getMessage(), elapsed);
        }
    }
