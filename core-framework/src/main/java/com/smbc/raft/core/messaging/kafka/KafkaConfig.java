...
/**
 * Resolved Kafka connection settings. Values are read from {@link ConfigurationManager} under
 * either {@code kafka.*} (default client) or {@code kafka.<name>.*} (named client). Arbitrary
 * producer or consumer properties may be passed through using {@code kafka.producer.*} / {@code
 * kafka.consumer.*}.
 */
@Data
@Builder
public class KafkaConfig {
  ...
  public static KafkaConfig fromConfig(String clientName) {
    ...
    String prefix =
        (clientName == null || clientName.equals("default")) ? "kafka" : "kafka." + clientName;
    ...
    return KafkaConfig.builder()
        ...
        .groupId(
            config.getProperty(
                prefix + ".group.id", "test-automation-" + System.currentTimeMillis()))
        ...
        .valueSerializer(
            config.getProperty(
                prefix + ".value.serializer",
                "org.apache.kafka.common.serialization.StringSerializer"))
        .keyDeserializer(
            config.getProperty(
                prefix + ".key.deserializer",
                "org.apache.kafka.common.serialization.StringDeserializer"))
        .valueDeserializer(
            config.getProperty(
                prefix + ".value.deserializer",
                "org.apache.kafka.common.serialization.StringDeserializer"))
