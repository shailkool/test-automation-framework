...
/**
 * Aggregate outcome of a {@link DirectoryDiff} run: one {@link FileDiffResult} per matched file
 * pair (or unmatched orphan), plus aggregated counts that drive the summary report.
 */
@Getter
public class DirectoryDiffResult {
  ...
  public DirectoryDiffResult(
      ...
      List<FileDiffResult> fileResults) {
    this.oldDir = oldDir;
    this.newDir = newDir;
  ...
  public int getIdenticalFiles() {
    return countByStatus(FileStatus.IDENTICAL);
  }

  public int getFilesWithDifferences() {
    return countByStatus(FileStatus.DIFFERENT);
  }

  public int getOnlyOldFiles() {
    return countByStatus(FileStatus.ONLY_OLD);
  }

  public int getOnlyNewFiles() {
    return countByStatus(FileStatus.ONLY_NEW);
  }
  ...
  /**
   * Overall match percentage across every compared file pair, weighted by the row count on each
   * side. Uncompared files (only-old/only-new) count as zero match. Returns 100.0 when nothing was
   * compared.
   */
  public double getOverallMatchPercentage() {
    long unchanged = getTotalUnchangedRows();
    long denom =
        fileResults.stream().mapToLong(f -> Math.max(f.getOldRowCount(), f.getNewRowCount())).sum();
