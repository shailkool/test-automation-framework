...
/**
 * Configuration for AutoSys job monitoring in a named environment. Declared under the "autosys" key
 * in the environment JSON.
 *
 * <p>Supports three query modes: CLI (autorep), REST API, or direct DB. Set "queryMode" to "CLI",
 * "REST", or "DB" to select.
 */
@Data
@NoArgsConstructor
...
public class AutosysJobSettings {
  ...
  public static class AutosysJobGroup {
    ...
    /**
     * Expected terminal status after the job runs. SUCCESS (default) | FAILURE | INACTIVE | RUNNING
     * | ON_HOLD | ON_ICE
     */
    private String expectedStatus = "SUCCESS";

    /**
     * If true, a non-SUCCESS status is a hard failure. If false, it is a warning (DEGRADED) that
