 ...
 * }</pre>
 *
 * <p>Exit codes: {@code 0} success/no-differences, {@code 1} differences detected, {@code 2}
...
public final class DiffCli {
  ...
  public static int run(String[] args, PrintStream out, PrintStream err) {
    ...
    try {
      switch (cmd) {
        case "file":
          return runFile(opts, out, err);
        case "dir":
          return runDir(opts, out, err);
        case "help":
        case "--help":
        case "-h":
          printUsage(out);
  ...
  static int runFile(Map<String, String> opts, PrintStream out, PrintStream err) {
    ...
    new DiffReportGenerator().saveReport(result, title, oldSide, newSide, outPath.toString());

    out.printf(
        "Compared %s vs %s - %s. Report: %s%n",
