...
/**
 * Resolves a {@link ConnectionFactory} for the configured provider.
 * Provider-specific classes are loaded reflectively so that a test run only
 * needs the broker client it actually uses on the classpath.
 */
final class JmsConnectionFactoryProvider {
    ...
    static ConnectionFactory create(JmsConfig config) {
        switch (config.getProvider()) {
            ...
            case CUSTOM:
            default:
                throw new MessagingException("CUSTOM JMS provider requires a ConnectionFactory "
    ...
    private static ConnectionFactory createActiveMq(JmsConfig config) {
        ...
        try {
            ...
            Object factory = factoryClass.getConstructor(String.class)
                    .newInstance(config.getBrokerUrl());
            if (config.getUsername() != null) {
                factoryClass.getMethod("setUserName", String.class)
                        .invoke(factory, config.getUsername());
            }
            if (config.getPassword() != null) {
                factoryClass.getMethod("setPassword", String.class)
                        .invoke(factory, config.getPassword());
            }
            return (ConnectionFactory) factory;
        } catch (ClassNotFoundException e) {
            throw new MessagingException("ActiveMQ client not on classpath. Add "
                    + "org.apache.activemq:activemq-client-jakarta to your dependencies.", e);
    ...
    private static ConnectionFactory createIbmMq(JmsConfig config) {
        ...
        try {
            ...
            factoryClass.getMethod("setQueueManager", String.class)
                    .invoke(factory, config.getMqQueueManager());
            factoryClass.getMethod("setChannel", String.class).invoke(factory, config.getMqChannel());

            Object transportType = constantsClass.getField(
                    "WMQ_CM_" + config.getMqTransportType().toUpperCase()).get(null);
            factoryClass.getMethod("setTransportType", int.class).invoke(factory, transportType);

            return (ConnectionFactory) factory;
        } catch (ClassNotFoundException e) {
            throw new MessagingException("IBM MQ client not on classpath. Add "
                    + "com.ibm.mq:com.ibm.mq.allclient to your dependencies.", e);
        } catch (ReflectiveOperationException e) {
