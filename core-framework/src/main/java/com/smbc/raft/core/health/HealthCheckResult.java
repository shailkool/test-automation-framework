 ...
 * Status is deliberately a simple three-state enum so the Spring Boot
 * layer you add later can map it to HTTP 200/207/503 without knowing
...
public class HealthCheckResult {
    ...
    public String summary() {
        return String.format("[%s] %s — %s (%dms): %s",
            category, name, status, durationMillis, detail);
    }
