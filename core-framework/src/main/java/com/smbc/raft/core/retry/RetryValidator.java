...
/**
 * Provides retry/loop mechanism for validating expected vs actual results Supports validation from
 * UI, API, and Database sources
 */
@Log4j2
public class RetryValidator {
  ...
  public static <T> ValidationResult<T> validateWithRetry(
      T expected, Supplier<T> actualSupplier, LoopType loopType, String description) {
    ...
    for (int i = 0; i < iterations; i++) {
      ...
      try {
        ...
        log.debug(
            "Attempt {}/{}: Expected={}, Actual={}", attemptNumber, iterations, expected, actual);
        ...
        if (matched) {
          ...
          log.info(
              "Match found on attempt {}/{} after {}ms", attemptNumber, iterations, elapsedTime);
          ExtentReportManager.logPass(
              String.format(
                  "Validation passed on attempt %d/%d (elapsed: %dms)",
    ...
    ValidationResult<T> result =
        new ValidationResult<>(
            expected, actual, matched, attemptNumber, iterations, totalElapsedTime, description);

    if (!matched) {
      log.warn(
          "Validation failed after {} attempts ({}ms): Expected={}, Actual={}",
          iterations,
          totalElapsedTime,
          expected,
          actual);
      ExtentReportManager.logFail(
          String.format(
              "Validation failed after %d attempts (%dms). Expected: %s, Actual: %s",
  ...
  public static ValidationResult<Boolean> validateConditionWithRetry(
      Supplier<Boolean> conditionSupplier, LoopType loopType, String description) {
    ...
    for (int i = 0; i < iterations; i++) {
      ...
      try {
        ...
        if (matched) {
          long elapsedTime = System.currentTimeMillis() - startTime;
          log.info(
              "String contains match found on attempt {}/{} after {}ms",
              attemptNumber,
              iterations,
              elapsedTime);
          ExtentReportManager.logPass(
              String.format(
                  "String contains validation passed on attempt %d/%d", attemptNumber, iterations));
          break;
        }
  ...
  public static boolean waitUntil(
    ...
    ValidationResult<Boolean> result =
        validateConditionWithRetry(conditionSupplier, loopType, description);
    return result.isMatched();
  }

  /** Wait until element is visible (for UI) */
  public static boolean waitUntilVisible(Supplier<Boolean> visibilitySupplier, LoopType loopType) {
  ...
  public static boolean waitUntilApiStatus(
      int expectedStatus, Supplier<Integer> statusSupplier, LoopType loopType) {

    ValidationResult<Integer> result =
        validateWithRetry(expectedStatus, statusSupplier, loopType, "API status code");
    return result.isMatched();
  }

  /** Wait until database record exists */
  public static boolean waitUntilRecordExists(
      Supplier<Boolean> existsSupplier, LoopType loopType, String tableName) {

    return waitUntil(existsSupplier, loopType, "Record exists in " + tableName);
  }
}
