...
public class PlaywrightManager {
  ...
  public static void initializeBrowser(BrowserEngine browserEngine, String channel) {
    try {
      ...
      Browser.NewContextOptions contextOptions =
          new Browser.NewContextOptions().setViewportSize(1920, 1080).setAcceptDownloads(true);
  ...
  public static Browser getBrowser() {
    return BrowserPool.getBrowser(
        BrowserEngine.valueOf(config.getBrowser().toUpperCase(java.util.Locale.ROOT)), null);
  }
  ...
  /**
   * Close the current context and page. The underlying pooled browser process remains alive for the
   * next test.
   */
