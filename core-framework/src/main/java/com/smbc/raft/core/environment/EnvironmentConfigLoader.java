 ...
 *
 * <ol>
 *   <li>Explicit absolute / relative filesystem path passed to {@link #load(String, String)}
 *   <li>Classpath resource at {@code <baseDir>/<env>.json} (default baseDir is {@code
 *       environments})
 *   <li>Filesystem file at {@code <baseDir>/<env>.json}
 * </ol>
 *
 * <p>The loader is stateless; {@link EnvironmentContext} layers singleton caching on top.
 */
@Log4j2
public class EnvironmentConfigLoader {
  ...
  public EnvironmentConfig load(String environmentName, String baseDir) {
    ...
    if (classpathStream != null) {
      try (InputStream in = classpathStream) {
        ...
        log.info("Loaded environment '{}' from classpath resource '{}'", environmentName, resource);
        return config;
      } catch (IOException e) {
    ...
    if (Files.exists(fsPath)) {
      try {
        ...
        applyDefaults(config, environmentName);
        log.info(
            "Loaded environment '{}' from filesystem '{}'",
            environmentName,
  ...
  /**
   * Load {@code <environmentName>.json} from a specific filesystem directory. Used when a run
   * profile points the loader at an external location (e.g. a secure volume mounted outside the
   * repository).
   */
