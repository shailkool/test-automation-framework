...
/**
 * Wrapper around {@link KafkaConsumer} that subscribes on demand and drains records into framework
...
public class KafkaConsumerClient implements AutoCloseable {
  ...
  /**
   * Poll the configured topic until at least {@code maxMessages} have been received or {@code
   * timeout} elapses. Returns whatever was drained.
   */
  public List<Message> poll(String topic, int maxMessages, Duration timeout) {
    ...
    log.debug(
        "Kafka consumer [{}] polled {} records from {}",
        config.getClientName(),
        received.size(),
        topic);
    return received;
  ...
  /**
   * Reset this consumer group to the earliest offset for the subscribed topic. Useful to replay
   * from the beginning during a test.
   */
