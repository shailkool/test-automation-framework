...
/**
 * LoopConfig variant that doubles the sleep interval on each failed attempt, capped at
 * maxIntervalMillis.
 */
public class ExponentialBackoffLoopConfig extends LoopConfig {
  ...
  public ExponentialBackoffLoopConfig(
  ...
  public long nextIntervalMillis() {
    long interval = Math.min(initialIntervalMillis * (1L << currentIteration), maxIntervalMillis);
    currentIteration++;
    return interval;
  }
}
