...
/**
 * Post-run hook that converts Cucumber's JSON output into a masterthought multi-tab HTML report.
 *
 * <p>Registered from {@link TaggedCucumberRunner}'s static initialiser as a JVM shutdown hook so it
 * fires after Cucumber has finished flushing {@code cucumber.json} regardless of how the runner was
 * launched (Maven, IDE, or {@code java -jar}).
 *
 * <p>If the JSON file does not exist (e.g. no scenarios matched the tag filter) the generator logs
 * a warning and exits quietly rather than failing the build.
 */
@Log4j2
public final class MasterthoughtReportGenerator {
  ...
  /**
   * masterthought appends its own {@code cucumber-html-reports} subdirectory to whatever is
   * configured here, so {@code test-output} produces a final path of {@code
   * test-output/cucumber-html-reports/}.
   */
  ...
  public static synchronized void registerShutdownHook() {
    ...
    Runtime.getRuntime()
        .addShutdownHook(
            new Thread(MasterthoughtReportGenerator::generate, "masterthought-report-generator"));
    registered = true;
  }
  ...
  public static void generate() {
    ...
    try {
      ...
      log.info(
          ...
          new File(new File(outputDir, REPORT_SUBDIR), "overview-features.html").getAbsolutePath());
    } catch (Exception e) {
  ...
  private static void addEnvironmentClassifications(Configuration configuration) {
    try {
      com.smbc.raft.core.environment.EnvironmentConfig config = EnvironmentContext.get();
  ...
  private static void addRunProfileClassifications(Configuration configuration) {
    try {
      ...
      if (profile.hasBrowser()) {
        ...
        String browser =
            profile.resolveBrowserEngine() + (channel == null ? "" : " (" + channel + ")");
        ...
        configuration.addClassifications("Screenshot mode", profile.resolveScreenshotMode().name());
      }

      Path externalDir = ctx.getEnvironmentConfigDir();
      if (externalDir != null) {
        configuration.addClassifications("External env-config dir", externalDir.toString());
      }
    } catch (RuntimeException e) {
