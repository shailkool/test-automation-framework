...
/**
 * {@link HealthCheck} implementation that queries one or more AutoSys jobs and returns an
 * aggregated {@link HealthCheckResult}.
 *
 * <p>Supports three query modes declared in the environment JSON:
 *
 * <ul>
 *   <li><b>CLI</b> — invokes {@code autorep -j <name> -s} via Runtime.exec
 *   <li><b>REST</b> — calls the AutoSys WCC REST API (r11.3.6+)
 *   <li><b>DB</b> — queries the {@code ujo_job} view via DatabaseManager
 * </ul>
 *
 * <p>Each instance covers one named job group (a list of job names that must all reach their
 * expected status). Register one check per group in the {@link HealthCheckRegistry}.
 *
 * <p>Usage:
 *
 * <pre>
...
public class AutosysJobCheck implements HealthCheck {
  ...
  public AutosysJobCheck(
      String checkName, AutosysJobSettings settings, AutosysJobSettings.AutosysJobGroup group) {
  ...
  public HealthCheckResult run() {
    ...
    if (group.getJobs() == null || group.getJobs().isEmpty()) {
      return result(HealthCheckResult.Status.SKIPPED, "No jobs configured in group", start);
    }
      ...
      return switch (settings.getQueryMode().toUpperCase()) {
        ...
        default ->
            result(
                ...
                "Unknown queryMode: " + settings.getQueryMode(),
                start);
      };
    } catch (Exception e) {
      log.error("AutosysJobCheck [{}] failed: {}", checkName, e.getMessage(), e);
      return result(HealthCheckResult.Status.DOWN, "Unexpected error: " + e.getMessage(), start);
    }
  }
   ...
   *
   * <p>Job Name Last Start Last End ST/Ex -------------------------------- --------------------
   * ------------ ----- TRADE_FEED_EXTRACT 03/15/2025 02:00:00 03/15 02:14 SU/0
   *
   * <p>The status code is in the "ST" column — two letters before the slash.
   */
    ...
    try {
      ...
      try (BufferedReader reader =
        ...
        while ((line = reader.readLine()) != null) {
          // Skip header lines
          if (line.isBlank() || line.startsWith("---") || line.startsWith("Job Name")) continue;
          ...
          if (line.contains(jobName)) {
            ...
            if (parts.length >= 1) {
              ...
              statusCode = last.contains("/") ? last.split("/")[0] : last;
            }
          }
        }
      }
  ...
  /**
   * AutoSys WCC REST API endpoint (r11.3.6+): GET /CA7/api/job/{jobName}/status
   *
   * <p>Requires a session token obtained via POST /CA7/api/login. Uses basic RestAssured directly
   * to keep the health library free of the application-automation layer.
   */
    ...
    try {
      io.restassured.response.Response response =
          io.restassured.RestAssured.given()
              ...
              .body(
                  "{\"username\":\""
                      + settings.getRestUsername()
                      + "\",\"password\":\""
                      + resolvePassword(settings.getRestPassword())
                      + "\"}")
              .post("/CA7/api/login");
  ...
  /**
   * Queries the AutoSys database view directly. The standard view is "ujo_job" — available in all
   * AutoSys versions.
   *
   * <p>Column used: "status" (integer in older versions, varchar in newer). Integer status codes:
   * 1=INACTIVE, 3=WAITING, 5=ON_HOLD, 7=ON_ICE, 9=ACTIVATED, 11=STARTING, 13=RUNNING, 15=SUCCESS,
   * 17=FAILURE
   */
  ...
  private AutosysJobStatus queryViaDb(DatabaseManager db, String jobName) {
    try {
      ...
      List<Map<String, Object>> rows =
          db.executeQuery("SELECT status FROM ujo_job WHERE job_name = ?", jobName);
  ...
  private HealthCheckResult buildResult(List<String> failures, List<String> details, long start) {
    ...
    if (failures.isEmpty()) {
      return result(
          ...
          group.getDescription() + " — all jobs " + group.getExpectedStatus() + " | " + detail,
          start);
    }

    HealthCheckResult.Status status =
        group.isCritical() ? HealthCheckResult.Status.DOWN : HealthCheckResult.Status.DEGRADED;
  ...
  private HealthCheckResult result(HealthCheckResult.Status status, String detail, long start) {
    return HealthCheckResult.builder()
        .name(checkName)
        .category("AutosysJob")
        .status(status)
