 ...
 *
 * <p>Delegates HTML rendering to {@link DiffHtmlReportGenerator} while giving tests a stable,
 * title-aware entry point. Ensures parent directories are created so callers can drop reports into
...
public class DiffReportGenerator {
  ...
  /**
   * Render {@code result} to HTML and write it to {@code outputPath} using the supplied labels for
   * the report's old/new file metadata strip.
   */
  public void saveReport(
      DiffResult result, String title, String oldLabel, String newLabel, String outputPath) {
    saveReport(
        result, title, DiffSideInfo.label(oldLabel), DiffSideInfo.label(newLabel), outputPath);
  }

  /**
   * Render {@code result} using rich per-side metadata and write the resulting HTML to {@code
   * outputPath}.
   */
  public void saveReport(
      DiffResult result,
    ...
    String html =
        DiffHtmlReportGenerator.generateHtml(result.getDataDiff(), title, oldSide, newSide);
  ...
  /**
   * Render {@code result} and return the HTML as a string without writing to disk. Useful for
   * embedding in email bodies or reporting dashboards.
   */
  public String generateHtml(DiffResult result, String title, String oldLabel, String newLabel) {
    if (result == null) {
      throw new IllegalArgumentException("DiffResult must not be null");
    }
    return DiffHtmlReportGenerator.generateHtml(result.getDataDiff(), title, oldLabel, newLabel);
  }
