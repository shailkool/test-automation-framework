 ...
 *
 * <ol>
 *   <li>Classpath resource {@code <baseDir>/<profile>.json} (default baseDir is {@code profiles})
 *   <li>Filesystem file {@code <baseDir>/<profile>.json}
 * </ol>
 */
@Log4j2
  ...
  public RunProfile load(String profileName, String baseDir) {
    ...
    if (classpathStream != null) {
      try (InputStream in = classpathStream) {
        ...
        log.info("Loaded run profile '{}' from classpath resource '{}'", profileName, resource);
        return profile;
      } catch (IOException e) {
    ...
    if (Files.exists(fsPath)) {
      try {
        ...
        applyDefaults(profile, profileName);
        log.info(
            "Loaded run profile '{}' from filesystem '{}'", profileName, fsPath.toAbsolutePath());
        return profile;
      } catch (IOException e) {
        throw new IllegalStateException(
