...
/**
 * Manages the lifecycle of Playwright Browser processes. Implements a pooled approach to reuse the
 * heavy browser process across multiple threads/tests while providing isolation via
 * BrowserContexts.
 */
@Log4j2
public class BrowserPool {
  ...
  /**
   * Gets a shared Browser instance for the given engine and channel. Reuses the browser process if
   * it already exists for the engine/channel combination.
   */
  public static synchronized Browser getBrowser(
      PlaywrightManager.BrowserEngine engine, String channel) {
  ...
  private static Browser launchBrowser(PlaywrightManager.BrowserEngine engine, String channel) {
    ...
    if (channel != null
        && !channel.isBlank()
        && engine == PlaywrightManager.BrowserEngine.CHROMIUM) {
      options.setChannel(channel);
    }

    log.info(
        "Launching shared Browser process: engine={}, channel={}, headless={}",
        engine,
        channel,
        CONFIG.isHeadless());
  ...
  /**
   * Closes all active browser processes and cleans up the Playwright instance. Should be called at
   * the end of the test suite (e.g., in a Global Shutdown Hook).
   */
