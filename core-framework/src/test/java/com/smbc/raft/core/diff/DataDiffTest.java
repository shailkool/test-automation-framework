...
public class DataDiffTest extends BaseTest {
  ...
  public void testGenerateHtmlReport() {
    ...
    DiffHtmlReportGenerator.generateReport(
        diff, reportPath, "Expected Products", "Actual Products");

    // Verify report was created
    Assert.assertTrue(new java.io.File(reportPath).exists(), "HTML report should be created");
  ...
  public void testCsvFileDiff() {
    ...
    DiffHtmlReportGenerator.generateReport(
        diff, reportPath, "Expected CSV: " + expectedCsv, "Actual CSV: " + actualCsv);
  ...
  public void testDatabaseResultDiff() {
    ...
    DiffHtmlReportGenerator.generateReport(
        diff, reportPath, "Expected Order Status", "Actual Database Results");

    // Verify differences
    Assert.assertEquals(diff.getModifiedRows().size(), 2, "Two orders should have status changes");
    Assert.assertEquals(diff.getAddedRows().size(), 1, "One new order should be added");
  ...
  public void testApiResponseDiff() {
    ...
    actualResponse.add(
        createApiResponse("2", "Error", "User not found")); // Different status and message
    ...
    DiffHtmlReportGenerator.generateReport(
        diff, reportPath, "Expected API Response", "Actual API Response");
  ...
  public void testCompositeKeyDiff() {
    ...
    DataDiff diff =
        new DataDiff(
            expected, actual, Arrays.asList("FirstName", "LastName") // Composite key
            );
  ...
  public void testCompositeKeyViaBuilder() {
    ...
    Assert.assertEquals(
        result.getMatchedRows(), 2, "John and Jane should be unchanged (case + ignored field)");
    ...
    Assert.assertEquals(
        result.getModifiedRows(), 0, "Nothing modified once ignoreCase + ignoreField applied");

    ExtentReportManager.logPass(
        "Composite-key builder diff completed: " + result.getSummaryString());
  }
  ...
  private Map<String, String> createPersonRow(
      String firstName, String lastName, String age, String country) {
    Map<String, String> row = new LinkedHashMap<>();
