...
import com.automation.core.messaging.kafka.KafkaMessageClient;
...
/**
 * Examples showing how to produce and consume Kafka messages through the
 * framework. Tagged as "kafka" so runs against a live broker are opt-in.
 */
public class KafkaMessagingTest extends BaseTest {
    ...
    public void testConsumeKafkaRecord() {
        ...
        kafka.send(Message.builder()
                .destination(TOPIC)
                .key(correlationKey)
                .payload("{\"status\":\"OK\"}")
                .build());

        Message received = kafka.receiveMatching(TOPIC,
                msg -> correlationKey.equals(msg.getKey()),
