 ...
 *
 * <p>Two modes shown: - on-demand: run checks once inside a @Test method (smoke/nightly) -
    ...
    if (services != null) {
      services.forEach(
          (key, svc) ->
              registry.register(
                  key,
                  new WindowsServiceCheck(svc.getDisplayName(), svc.getName(), svc.getHost())));
    }
    ...
    if (filePaths != null) {
      filePaths.forEach(
          (key, fp) ->
              registry.register(
                  key,
                  FilePresenceCheck.builder()
                      .name(key)
                      .path(fp.getPath())
                      .expectedFormats(
                          fp.getExpectedFormats() != null
                              ? Set.copyOf(fp.getExpectedFormats())
                              : Set.of())
    ...
    while (pollCount < totalPolls) {
      ...
      ExtentReportManager.logInfo(
          String.format(
              ...
              pollCount,
              totalPolls,
              results.size() - failCount,
              results.size(),
              results.isEmpty() ? "N/A" : results.get(0).timestamp()));

      if (failCount > 0) {
        degraded++;
        registry.failures().forEach(r -> ExtentReportManager.logWarning("  " + r.summary()));
      }
    }
