package com.smbc.raft.tests.validation;
...
import com.smbc.raft.core.retry.LoopConfig;
...
public class ValidationLoopExamplesTest extends BaseTest {
  ...
  public void testAPIValidationOnce() {
    ...
    // Create a user
    Response createResponse = userApi.createUser("Test User", "testuser@example.com", "user");
  ...
  public void testDatabaseValidationLongLoop() {
    ...
    ExtentReportManager.logInfo(
        "User processed after " + String.format("%.1f", result.getDurationSeconds()) + " seconds");
  ...
  public void testCustomValidationLoop() {
    ...
    ValidationResult<Response> result =
        loop.validateCondition(
            () -> userApi.getAllUsers(), response -> response.getStatusCode() == 200);

    Assert.assertTrue(result.isSuccess());
    ExtentReportManager.logPass(
        "Custom validation completed in " + result.getDurationMillis() + " ms");
  }
  ...
  public void testValidateResponseContains() {
    ...
    Response createResponse = userApi.createUser(uniqueName, "searchuser@example.com", "user");
    int userId = createResponse.jsonPath().getInt("id");
  ...
  public void testValidateDatabaseCount() {
    ...
    ValidationResult<Integer> result =
        loop.validateEquals(() -> userDb.getUserCount(), initialCount + 1);
  ...
  public void testValidationFailure() {
    ...
    ValidationResult<Response> result =
        loop.validateEquals(() -> userApi.getUserById(999999), null);
    ...
    ExtentReportManager.logInfo(
        "Validation failed as expected after " + result.getIterations() + " attempts");
  }
  ...
  public void testEndToEndValidation() {
    ...
    ValidationResult<Boolean> dbResult = dbLoop.validateTrue(() -> userDb.emailExists(email));
    ...
    ValidationResult<Response> apiResult =
        apiLoop.validateCondition(
            () -> userApi.getUserById(userId),
            response ->
