...
public class DataDrivenUserApiTest extends BaseTest {
  ...
  public void testCreateUserWithCSVData(Map<String, String> testData) {
    ...
    // Verify status code
    Assert.assertEquals(
        response.getStatusCode(), expectedStatus, "Status code mismatch for: " + description);
    ...
    if (response.getStatusCode() == 201) {
      ...
      ExtentReportManager.logPass(
          "Request failed as expected with status: " + response.getStatusCode());
    }
  }
  ...
  public void testValidUserCreation() {
    ...
    for (Object[] data : testData) {
      ...
      Assert.assertEquals(response.getStatusCode(), 201, "User creation should succeed");
