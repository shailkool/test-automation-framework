...
/**
 * Examples showing how to send and receive via JMS (ActiveMQ by default, IBM MQ when configured).
 * Tagged as "jms" so it only runs when enabled.
 */
public class JmsMessagingTest extends BaseTest {
  ...
  @Test(
      groups = {"jms", "nightly"},
      description = "Send a JMS message to a queue")
  ...
  @Test(
      groups = {"jms", "nightly"},
      description = "Receive a JMS message matching correlation id",
  ...
  public void testReceiveJmsMessage() {
    ...
    Message received =
        jms.receiveMatching(QUEUE, msg -> "corr-1".equals(msg.getKey()), Duration.ofSeconds(10));
    ...
    Assert.assertEquals(
        received.getHeader("x-source"), "test-automation", "Custom header should round-trip");
    ExtentReportManager.logPass("Received JMS message " + received.getMessageId());
  }

  @Test(
      groups = {"jms", "nightly"},
      description = "IBM MQ via named client (requires QA env)")
