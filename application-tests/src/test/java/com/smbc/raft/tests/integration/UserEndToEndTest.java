...
@Test(
    singleThreaded = true,
    description = "Integration tests for user lifecycle across UI, API and DB layers")
public class UserEndToEndTest extends BaseTest {
  ...
  @Test(
      groups = {"regression"},
      priority = 1,
      description = "End-to-end test: Create user via API and verify in database")
  ...
  @Test(
      groups = {"regression"},
      priority = 2,
      description = "End-to-end test: Update user via API and verify in database",
  ...
  public void testUpdateUserApiToDatabase() {
    ...
    Assert.assertEquals(
        apiResponse.jsonPath().getString("name"),
        newName,
  ...
  @Test(
      groups = {"regression"},
      priority = 3,
      description = "End-to-end test: Verify user can login via UI",
      dependsOnMethods = "testCreateUserApiToDatabase")
  public void testUserLoginViaUI() {
    ...
    Assert.assertTrue(
        userDb.emailExists(testUserEmail), "User should exist in database before login");
  ...
  @Test(
      groups = {"regression"},
      priority = 4,
      description = "End-to-end test: Delete user via API and verify in database",
  ...
  public void testDeleteUserApiToDatabase() {
    ...
    Assert.assertTrue(
        apiResponse.getStatusCode() == 200 || apiResponse.getStatusCode() == 204,
    ...
    Assert.assertFalse(
        userDb.emailExists(testUserEmail), "User should not exist in database after deletion");

    // Step 4: Verify user cannot be retrieved via API
    Response getResponse = userApi.getUserById(testUserId);
    Assert.assertEquals(getResponse.getStatusCode(), 404, "Getting deleted user should return 404");
  ...
  @Test(
      groups = {"nightly"},
      description =
          "Complete workflow: API creation → DB verification → UI interaction → API cleanup")
  public void testCompleteUserLifecycle() {
    ...
    Assert.assertTrue(
        deleteResponse.getStatusCode() == 200 || deleteResponse.getStatusCode() == 204);
