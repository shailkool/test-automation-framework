package com.smbc.raft.tests.runprofile;
...
import com.smbc.raft.core.environment.EnvironmentConfigLoader;
...
/**
 * Exercises the agnostic run-profile plumbing against the local/ci/external profile JSONs shipped
 * under {@code src/test/resources/profiles}.
 */
public class RunProfileContextTest {
  ...
  public void ciProfileSelectsHeadlessChromiumWithoutChannel() {
    ...
    RunProfile profile = ctx.getProfile();
    assertEquals(profile.resolveBrowserEngine(), BrowserEngine.CHROMIUM);
    assertNull(profile.resolveBrowserChannel(), "bundled chromium should not carry a channel");
  ...
  public void externalConfigProfileExposesEdgeChannelAndExternalDir() {
    ...
    assertEquals(ctx.getEnvironmentConfigDir().toString(), "/opt/secure-configs/environments");
  }
  ...
  @Test
  public void runProfileOverridesEnvironmentConfigDirectory() throws Exception {
    ...
    try {
      ...
      EnvironmentConfig direct = new EnvironmentConfigLoader().loadFromDir("stag", tempDir);
