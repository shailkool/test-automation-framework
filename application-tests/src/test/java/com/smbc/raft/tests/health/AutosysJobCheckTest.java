...
/**
 * AutoSys job status validation. Covers two patterns: - snapshot: assert jobs are SUCCESS right now
...
public class AutosysJobCheckTest extends BaseTest {
  ...
  public void testAutosysJobsSnapshot() {
    ...
    for (var schedulerEntry : autosysMap.entrySet()) {
      ...
      for (var groupEntry : settings.getJobGroups().entrySet()) {
        AutosysJobCheck check =
            new AutosysJobCheck(groupEntry.getKey(), settings, groupEntry.getValue());
  ...
  public void testDailyFeedJobsCompleteWithinTimeout() {
    ...
    AutosysJobSettings.AutosysJobGroup feedGroup = settings.getJobGroups().get("dailyFeed");
