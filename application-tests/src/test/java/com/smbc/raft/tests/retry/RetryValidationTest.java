...
public class RetryValidationTest extends BaseTest {
  ...
  public void testApiValidationWithShortLoop() {
    ...
    Assert.assertTrue(
        result.isMatched(), "API should return 200 status code: " + result.getSummary());
  ...
  public void testDatabaseValidationWithLongLoop() {
    ...
    boolean recordExists =
        RetryValidator.waitUntilRecordExists(
            () -> userDb.emailExists(email), LoopType.LONG, "users");
  ...
  public void testUiValidationOnce() {
    ...
    Assert.assertTrue(result.isMatched(), "Page title should match: " + result.getSummary());
  ...
  public void testCustomLoopValidation() {
    ...
    ValidationResult<Integer> result =
        RetryValidator.validateWithCustomLoop(
            200,
    ...
    ExtentReportManager.logInfo(
        String.format(
            "Validation succeeded in attempt %d/%d (%.2fs)",
            result.getAttemptNumber(), result.getTotalAttempts(), result.getElapsedTimeSeconds()));
  }
  ...
  public void testStringContainsValidation() {
    ...
    ValidationResult<String> result =
        RetryValidator.validateStringContainsWithRetry(
            email, // Expected substring
            () -> userApi.getUserById(userId).getBody().asString(), // Actual string
            LoopType.SHORT,
            "API response contains email");

    Assert.assertTrue(result.isMatched(), "Response should contain user email");
  ...
  public void testFailedValidationAfterRetries() {
    ...
    Assert.assertEquals(result.getAttemptNumber(), 3, "Should have attempted all 3 iterations");
  ...
  public void testWaitForApiStatus() {
    ...
    boolean success =
        RetryValidator.waitUntilApiStatus(
            200, // Expected status
