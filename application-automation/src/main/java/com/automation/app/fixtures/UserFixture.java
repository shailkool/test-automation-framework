...
import com.automation.core.data.TestDataRegistry;
...
/**
 * Fixture for creating User test data.
 * Use via TestDataFactory — don't instantiate directly.
 *
 * Example:
 *   UserFixture user = TestDataFactory.user().withRole("admin").create();
 *   // use user.getId(), user.getEmail() etc.
 *   // cleanup happens automatically in @AfterMethod via TestDataRegistry
 */
@Log4j2
@Getter
public class UserFixture {
    ...
    /**
     * Creates the user directly in the DB (useful for DB-layer tests
     * where the API isn't the system under test).
     */
