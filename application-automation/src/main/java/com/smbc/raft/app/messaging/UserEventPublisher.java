...
/**
 * Application-specific helper demonstrating how to publish and verify user-domain events on a Kafka
 * topic via the core messaging layer.
 */
@Log4j2
public class UserEventPublisher {
  ...
  public Message publishUserCreated(int userId, String email) {
    Message message =
        Message.builder()
            ...
            .payload(
                String.format(
                    "{\"type\":\"USER_CREATED\",\"id\":%d,\"email\":\"%s\"}", userId, email))
            .build()
    ...
    log.info("Published USER_CREATED event (userId={}, offset={})", userId, message.getOffset());
    return message;
  }
  ...
  public Message awaitEventForUser(int userId, Duration timeout) {
    return client.receiveMatching(
        topic, msg -> String.valueOf(userId).equals(msg.getKey()), timeout);
  }
}
